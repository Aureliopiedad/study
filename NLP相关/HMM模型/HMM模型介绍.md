...
- 在不同的时刻$t_1$和$t_2$，$x(t_1)$和$x(t_2)$用来表示一个隐含的状态，无法被观测者直接观察
- 在不同的时刻$t_1$和$t_2$，$y(t_1)$和$y(t_2)$用来表示一个可观测的状态，可以被观测值直接观察
- $y(t)$只和$x(t)$相关
- 受限于马尔可夫性质，$x(t)$和$x(t - 1)$有关，$x(t - 1)$和$x(t - 2)$有关
- 通常情况下，t的起始被设置为0或1
...
- 将$x(t)$认为是隐含的状态，$y(t)$认为是可观测的输出
- 认为$x(t)$的集合为：$Q=\{q_1,q_2,...,q_n\}$
- 认为$y(t)$的集合为：$V=\{v_1,v_2,...,v_m\}$
- 从$q_i$到$q_j$的概率是$a_{ij}$，代表从一个隐含状态到另一个隐含状态的转换概率
- 从$q_i$生成$v_j$的概率是$b_{ij}$，代表从一个隐含状态到一个可见状态的输出概率
...
HMM模型由于自身的限制，假设当前的观测值只和当前的隐藏状态相关，因此引入了MEMM模型，即最大熵隐马模型。同时，也是在MEMM中，引入了特征函数和权重的概念。

在序列的标注问题中，隐含状态的标注不仅和上一个字符有关，还和整个序列的其他上下文密不可分，这样就导致了标注的不全面。因此，引入了条件随机场(CRF，Conditional Random Field)的概念。