...
如果使用idea，可以去安装一个[antlr4的插件](https://plugins.jetbrains.com/plugin/7358-antlr-v4)。
...
如上文，一般来说，Token的命名始终以大写字母开头，规则名始终以小写字母开头。这个简单的语法规则表达的是：将传入的字符串解析成hello和其他字符串，忽略空格、制表符、换行符、回车。

例如，当输入为：'hello test'时，输出为：(r hello ID:test)。

## DemoV2

下面进行一个比较复杂的语句逻辑定义Express.g4：

```
grammar Expression;

@header{
package com.test1.expression;
}
...
expr
   ...
   | expr OP=(TIMES | DIVIDE) expr # expressionWithTimesOrDivide
   | expr OP=(PLUS | MINUS) expr # expressWithPlusOrMinus
   | NUMBER # expressWithNumber
  ;
...
```java
public class ExpressDoubleVisitor extends ExpressionBaseVisitor<Double> {
    @Override
    public Double visitExpressionWithBr(ExpressionParser.ExpressionWithBrContext ctx) {
        return visit(ctx.expr(0));
    }

    @Override
    public Double visitExpressWithPlusOrMinus(ExpressionParser.ExpressWithPlusOrMinusContext ctx) {
    ...
    @Override
    public Double visitExpressWithNumber(ExpressionParser.ExpressWithNumberContext ctx) {
        return Double.parseDouble(ctx.NUMBER().getText());
    }

    @Override
    public Double visitExpressionWithTimesOrDivide(ExpressionParser.ExpressionWithTimesOrDivideContext ctx) {
        Double left = visit(ctx.expr(0));
        Double right = visit(ctx.expr(1));
    ...
    @Override
    public Double visitCalc(ExpressionParser.CalcContext ctx) {
        return visit(ctx.expr(0));
    }
}
```

这样，当输入为：'(1 + 2) * (2 + 1)'时，输出为：'9.0'。

很好，你已经学会antlr的基本使用了，现在是时候查看siddhi的语法了。
